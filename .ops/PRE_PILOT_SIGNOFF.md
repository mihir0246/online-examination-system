...
- [ ] Redis persistence is enabled (AOF or RDB snapshots) to prevent data loss on restart.
...
- [ ] Rate limiting is configured and tested (e.g., max requests per student per minute) to prevent abuse during exams.
- [ ] Disaster recovery plan is documented with rollback procedures in case of critical issues during the pilot exam.
